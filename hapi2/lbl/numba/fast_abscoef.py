...
from .settings import FASTMATH, PARALLEL
from .sdv import PROFILE_SDVOIGT, PROFILE_LORENTZ, PROFILE_DOPPLER
from ....partsum import PYTIPS2017
from ....proxy import ISO, LOCAL_TABLE_CACHE
from ....legacy import DefaultIntensityThreshold,DefaultOmegaWingHW,\
                 listOfTuples,getDefaultValuesForXsect,getColumns, \
                 cBolts, cc
...
#ISO_INDEX_DEFAULT,MOL_INDEX = generate_indexes(ISO) # !!! THIS SHOULD BE DONE IN A DIFFERENT WAY TO BE ABLE TO UPDATE THE INDEX BEFORE EACH CALCULATION
...
def calculate_GammaD(N,T,LineCenterDB,MoleculeNumberDB,IsoNumberDB,ISO_INDEX,MOL_INDEX):
    ...
    for i in range(N):
        ...
        GammaD[i] = np.sqrt(2*cBolts*T*np.log(2)/m/cc**2)*LineCenterDB[i]
    return GammaD
...
def ENV_DEPENDENCE_ISO_INDEX(MOL_INDEX,ISO_INDEX,ISOS,T,Tref,partsum):
    """
...
#======================================================================
def absorptionCoefficientNumba_Voigt(Components=None,SourceTables=None,partitionFunction=PYTIPS2017,
    ...
    MOLEC_ID,LOCAL_ISO_ID = getColumns(TABLE_NAME,['molec_id','local_iso_id'])
    NLINES = len(MOLEC_ID)
    ...
    nu,xsc = ABSCOEF_FAST(NLINES,TABLE_NAME,ISOS,DILUENT,
                 OmegaRange=OmegaRange,OmegaStep=OmegaStep,
...
#========================================================================
def absorptionCoefficientNumba_Lorentz(Components=None,SourceTables=None,partitionFunction=PYTIPS2017,
                                Environment=None,OmegaRange=None,OmegaStep=None,OmegaWing=None,
                                IntensityThreshold=DefaultIntensityThreshold,
                                OmegaWingHW=DefaultOmegaWingHW,
                                GammaL='gamma_air', HITRAN_units=True, LineShift=True,
...
#========================================================================
def absorptionCoefficientNumba_Doppler(Components=None,SourceTables=None,partitionFunction=PYTIPS2017,
                                Environment=None,OmegaRange=None,OmegaStep=None,OmegaWing=None,
                                IntensityThreshold=DefaultIntensityThreshold,
                                OmegaWingHW=DefaultOmegaWingHW,
                                GammaL='gamma_air', HITRAN_units=True, LineShift=True,
...
def ABSCOEF_FAST(NLINES,TABLE_NAME,ISOS,DILUENT,
    ...
    OmegaRange = np.array(OmegaRange,dtype=np.float64)
    #OmegaRange = (float(OmegaRange[0]),float(OmegaRange[1]))
    #raise Exception('OmegaRange: ',OmegaRange,type(OmegaRange))
    OmegaStep = np.float64(OmegaStep)
    ...
    MOLEC_ID = LOCAL_TABLE_CACHE[TABLE_NAME]['data']['molec_id']
    LOCAL_ISO_ID = LOCAL_TABLE_CACHE[TABLE_NAME]['data']['local_iso_id']
        
    # Get centers, intensities, and lower states
    NU = LOCAL_TABLE_CACHE[TABLE_NAME]['data']['nu']
    SW = LOCAL_TABLE_CACHE[TABLE_NAME]['data']['sw']
    ELOWER = LOCAL_TABLE_CACHE[TABLE_NAME]['data']['elower']

    ISO_INDEX_DEFAULT,MOL_INDEX = generate_indexes(ISO)
    ...
    ISO_INDEX = ISO_INDEX_DEFAULT # stub
    
    ENV_DEPENDENCE_ISO_INDEX(MOL_INDEX,ISO_INDEX,ISOS,T,Tref,partsum) # go through ISO_INDEX  and change partition sums and current abundances
    ...
    for broadener,fraction in DILUENT:
        GAMMA_BR = LOCAL_TABLE_CACHE[TABLE_NAME]['data']['gamma_%s'%broadener]
        if broadener=='self': # !!! THIS SHOULD BE REDONE !!!
            N_BR = LOCAL_TABLE_CACHE[TABLE_NAME]['data']['n_air']
        else:
            N_BR = LOCAL_TABLE_CACHE[TABLE_NAME]['data']['n_%s'%broadener.lower()]
    ...
    for broadener,fraction in DILUENT:
        if broadener=='self': # !!! THIS SHOULD BE REDONE !!!
            ...
            DELTA_BR = LOCAL_TABLE_CACHE[TABLE_NAME]['data']['delta_%s'%broadener]
    ...
    if not test:
        ...
        Xsect = CALC_(Omegas,NU,SW,ELOWER,MOLEC_ID,LOCAL_ISO_ID,GAMMA_L,GAMMA_D,DELTA,NLINES,
                    OmegaRange,OmegaStep,OmegaWing,OmegaWingHW,reflect,profile)    
        #Xsect = CALC0_(Omegas,NU,SW,ELOWER,MOLEC_ID,LOCAL_ISO_ID,GAMMA_L,GAMMA_D,DELTA,NLINES,
        #            OmegaRange,OmegaStep,OmegaWing,OmegaWingHW,reflect,profile,NCORES)    
    else:
...
def CALC0_(Omegas,NU,SW,ELOWER,MOLEC_ID,LOCAL_ISO_ID,GAMMA_L,GAMMA_D,DELTA,NLINES,
          OmegaRange=None,OmegaStep=None,OmegaWing=None,OmegaWingHW=None,reflect=True,profile=1,NCORES=1):
